  ...
  return (
    <PhotoPostShell className="animeLeft">
      <form onSubmit={handleSubmit(onSubmit)} noValidate>
        <Input label="Nome" type="text" error={errors.nome?.message} {...register('nome')} />
        <Input label="Peso" type="number" error={errors.peso?.message} {...register('peso')} />
        <Input label="Idade" type="number" error={errors.idade?.message} {...register('idade')} />
        <FileLabel htmlFor="img">Imagem</FileLabel>
        <FileInput type="file" name="img" id="img" accept="image/*" onChange={handleImgChange} />
        <Error error={imgError} />
        {loading ? <Button disabled>Enviando...</Button> : <Button>Enviar</Button>}
        <Error error={error} />
      </form>
      <div>{img.preview && <Preview $imageUrl={img.preview} />}</div>
    </PhotoPostShell>
  );
};
